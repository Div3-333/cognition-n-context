...
interface MatrixViewProps {
  ...
  onDeleteLexiconEntry: (word: string) => void;
...
export const MatrixView: React.FC<MatrixViewProps> = ({
  breakdown, theme, currentTheme, onUpdateLexicon, onDeleteLexiconEntry, onMorphemeEdit, onFindConcordance, onSpeak
}) => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-3 gap-6">
      {breakdown.map((word, i) => (
        <div key={i} className={`flex flex-col ${currentTheme.panel} border border-white/5 rounded-2xl p-6 hover:border-indigo-500/30 transition-all group relative`}>
          <button 
            onClick={() => { if(confirm(`Delete analysis for "${word.word}"?`)) onDeleteLexiconEntry(word.word); }}
